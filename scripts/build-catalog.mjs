...
function main() {
  return Promise.all([
    ...
    const rows = enriched.map((c) =>
      `  { slug: ${JSON.stringify(c.slug)}, brand: ${JSON.stringify(c.brand)}, model: ${JSON.stringify(c.model)}, ` +
      `body: ${JSON.stringify(c.body)}, powertrain: ${JSON.stringify(c.powertrain)}, ` +
      `years: ${c.years ? `[${c.years[0]}, ${c.years[1] ?? "null"}]` : "null"}, ` +
      `generations: ${c.generations}, prominence: ${c.prominence.toFixed(4)} },`
    ).join("\n");
